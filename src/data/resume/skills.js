const skills = [
  {
    title: 'Javascript',
    competency: 4,
    category: ['Languages'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/javascript/javascript-original.svg',
  },
  {
    title: 'Node.JS',
    competency: 4,
    category: ['Backend', 'Javascript'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/nodejs/nodejs-plain-wordmark.svg',
  },
  {
    title: 'React',
    competency: 5,
    category: ['Frontend', 'Javascript'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/react/react-original-wordmark.svg',
  },
  {
    title: 'Angular',
    competency: 2,
    category: ['Frontend', 'Javascript'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/angular/angular-original.svg',
  },
  {
    title: 'VUE 3',
    competency: 5,
    category: ['Frontend', 'Javascript'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/vuejs/vuejs-original.svg',
  },
  {
    title: 'SpringBoot',
    competency: 3,
    category: ['Backend', 'Java'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/spring/spring-original.svg',
  },
  {
    title: 'Hibernate',
    competency: 3,
    category: ['Backend', 'Java'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/hibernate/hibernate-original-wordmark.svg',
  },
  {
    title: 'Next.JS',
    competency: 2,
    category: ['Frontend', 'Javascript'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/nextjs/nextjs-original-wordmark.svg',
  },
  {
    title: 'Bash',
    competency: 2,
    category: ['Development Tools', 'Languages'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/bash/bash-original.svg',
  },
  {
    title: 'MongoDB',
    competency: 2,
    category: ['Databases'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/mongodb/mongodb-original-wordmark.svg',
  },
  {
    title: 'PostgreSQL',
    competency: 3,
    category: ['Databases', 'Languages'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/postgresql/postgresql-original-wordmark.svg',
  },
  {
    title: 'MySQL',
    competency: 3,
    category: ['Databases', 'Languages'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/mysql/mysql-original-wordmark.svg',
  },
  {
    title: 'Redis',
    competency: 3,
    category: ['Databases'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/redis/redis-original-wordmark.svg',
  },
  {
    title: 'Express.JS',
    competency: 3,
    category: ['Backend', 'Javascript'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/express/express-original.svg',
  },
  {
    title: 'JUnit',
    competency: 2,
    category: ['Backend', 'Java'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/junit/junit-original.svg',
  },
  {
    title: 'Mocha',
    competency: 2,
    category: ['Backend', 'Javascript'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/mocha/mocha-original.svg',
  },
  {
    title: 'Redux',
    competency: 4,
    category: ['Frontend'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/redux/redux-original.svg',
  },
  {
    title: 'Pinia',
    competency: 4,
    category: ['Frontend', 'Javascript'],
    icon: 'https://miro.medium.com/v2/resize:fit:1400/1*lsZvtmCOGN6rUJAZubvuyg.png',
  },
  {
    title: 'TDesign',
    competency: 2,
    category: ['Frontend', 'Javascript'],
    icon: 'https://user-images.githubusercontent.com/7600149/147126818-0e5a5b2d-a833-4107-b654-442f3a1098c5.png',
  },
  {
    title: 'Antd',
    competency: 2,
    category: ['Frontend', 'Javascript'],
    icon: 'https://www.specbee.com/sites/default/files/inline-images/Ant.png',
  },
  {
    title: 'Bootstrap',
    competency: 2,
    category: ['Frontend', 'Javascript'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/bootstrap/bootstrap-original-wordmark.svg',
  },
  {
    title: 'Wasabi',
    competency: 2,
    category: ['Development Tools'],
    icon: 'https://www.gartner.com/pi/vendorimages/wasabi_public-cloud-storage-services-worldwide_1690550738668.png',
  },
  {
    title: 'Koa.JS',
    competency: 4,
    category: ['Backend', 'Javascript'],
    icon: 'https://raw.githubusercontent.com/koajs/koa/HEAD/docs/logo.png',
  },
  {
    title: 'MyBatis',
    competency: 3,
    category: ['Backend', 'Java'],
    icon: 'https://avatars.githubusercontent.com/u/1483254?s=280&v=4',
  },
  {
    title: 'FastAPI',
    competency: 2,
    category: ['Backend', 'Python'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/fastapi/fastapi-original.svg',
  },
  {
    title: 'Git',
    competency: 3,
    category: ['Development Tools'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/git/git-original-wordmark.svg',
  },
  {
    title: 'AWS S3',
    competency: 4,
    category: ['Development Tools'],
    icon: 'https://media.licdn.com/dms/image/D4D12AQFVwyOFW6vcWQ/article-cover_image-shrink_600_2000/0/1657367415533?e=2147483647&v=beta&t=Nkb1E45hxIPA9yhKMdt5DthjWvu2_TKOEDcKFimXdVQ',
  },
  {
    title: 'AWS EC2',
    competency: 4,
    category: ['Development Tools'],
    icon: 'https://cloudiofy.com/wp-content/uploads/2020/06/aws-ec2.png',
  },
  {
    title: 'AWS Lambda',
    competency: 3,
    category: ['Development Tools'],
    icon: 'https://miro.medium.com/v2/resize:fit:506/0*uoRkNnhqu3d61Zgx.png',
  },
  {
    title: 'AWS SES',
    competency: 3,
    category: ['Development Tools'],
    icon: 'https://static.javatpoint.com/tutorial/aws/images/amazon-ses.png',
  },
  {
    title: 'Docker',
    competency: 3,
    category: ['Development Tools'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/docker/docker-original.svg',
  },
  {
    title: 'Kafka',
    competency: 2,
    category: ['Backend'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/apachekafka/apachekafka-original-wordmark.svg',
  },
  {
    title: 'Numpy',
    competency: 3,
    category: ['Python'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/numpy/numpy-original-wordmark.svg',
  },
  {
    title: 'Tensorflow',
    competency: 3,
    category: ['Python'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/tensorflow/tensorflow-original-wordmark.svg',
  },
  {
    title: 'Keras',
    competency: 3,
    category: ['Python'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/keras/keras-original-wordmark.svg',
  },
  {
    title: 'PyTorch',
    competency: 3,
    category: ['Python'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/pytorch/pytorch-original-wordmark.svg',
  },
  {
    title: 'Jupyter',
    competency: 3,
    category: ['Python'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/jupyter/jupyter-original-wordmark.svg',
  },
  {
    title: 'Typescript',
    competency: 3,
    category: ['Frontend', 'Languages', 'Javascript'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/typescript/typescript-original.svg',
  },
  {
    title: 'HTML',
    competency: 3,
    category: ['Frontend', 'Languages'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/html5/html5-original.svg',
  },
  {
    title: 'CSS/SCSS/less',
    competency: 3,
    category: ['Frontend', 'Languages'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/css3/css3-original.svg',
  },
  {
    title: 'Python',
    competency: 4,
    category: ['Languages', 'Python'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/python/python-original.svg',
  },
  {
    title: 'Java',
    competency: 4,
    category: ['Languages'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/java/java-original.svg',
  },
  {
    title: 'C++',
    competency: 1,
    category: ['Languages'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/cplusplus/cplusplus-original.svg',
  },
  {
    title: 'MATLAB',
    competency: 2,
    category: ['Languages'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/matlab/matlab-original.svg',
  },
  {
    title: 'Pandas',
    competency: 3,
    category: ['Python'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/pandas/pandas-original-wordmark.svg',
  },
  {
    title: 'Matplotlib',
    competency: 3,
    category: ['Python'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/matplotlib/matplotlib-original-wordmark.svg',
  },
  {
    title: 'CouchDB',
    competency: 3,
    category: ['Databases'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/couchdb/couchdb-original.svg',
  },
  {
    title: 'Linux',
    competency: 3,
    category: ['Development Tools'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/linux/linux-original.svg',
  },
  {
    title: 'Nginx',
    competency: 3,
    category: ['Development Tools'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/nginx/nginx-original.svg',
  },
  {
    title: 'Django',
    competency: 3,
    category: ['Python', 'Backend'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/django/django-plain-wordmark.svg',
  },
  {
    title: 'Kubernates',
    competency: 1,
    category: ['Development Tools'],
    icon: 'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/kubernetes/kubernetes-original-wordmark.svg',
  },
].map((skill) => ({ ...skill, category: skill.category.sort() }));

// this is a list of colors that I like. The length should be === to the
// number of categories. Re-arrange this list until you find a pattern you like.
const colors = [
  '#6968b3',
  '#37b1f5',
  '#40494e',
  '#515dd4',
  '#e47272',
  '#cc7b94',
  '#3896e2',
  '#c3423f',
  '#d75858',
  '#747fff',
  '#64cb7b',
];

const categories = [
  ...new Set(skills.flatMap(({ category }) => category)),
].sort().map((category, index) => ({
  name: category,
  color: colors[index],
}));

export { categories, skills };
